-- Migration: add YouTube verification fields to songs
-- Adds:
...
create index if not exists idx_songs_youtube_verified
  on songs (band_id, youtube_verified)
  where youtube_verified = false;

comment on column songs.youtube_verified is
  'True once a band admin or member has confirmed the YouTube video match for this song.';
comment on column songs.youtube_video_id is
